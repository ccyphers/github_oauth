    ...
    function login_wrap(username, password) {
        return request.getAsync('https://github.com/login/oauth/authorize?client_id=' + client_id + '&state=' + state)
            ...
            .then(function (res) {
                ...
                var $redirect_body = cheerio.load(res[1]);

                if (res[1].match(/\?code=/)) {
                    return Promise.resolve({request: res[0], body: res[1], code: true});
                } else {
        ...
        return login_wrap(username, password)
            .then(function (res) {
                ...
                if (res.code) {
                    ...
                    var code = res.request.headers.location.split('code=')[1].split('&')[0];
                    var post_data = {
                        client_id: client_id,
                    ...
                    return request.postAsync({url: 'https://github.com/login/oauth/authorize', form: post_data})
                        .then(function (res) {
                            var code = res[0].headers.location.split('code=')[1].split('&')[0];
                            var post_data = {
                                client_id: client_id,
                                client_secret: client_secret,
                                code: code,
